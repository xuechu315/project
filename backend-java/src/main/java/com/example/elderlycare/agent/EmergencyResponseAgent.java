...
public class EmergencyResponseAgent {
     ...
     * 推送事件到后台工作人员队列（人工决策点）
     * 
     * 原 StaffNotificationAgent 已移除，改为记录日志并标记事件待处理状态
     * 后台工作人员可通过 getPendingEvents() 接口获取待处理事件列表
     */
    private void pushToStaffQueue(AlertEvent event) {
        log.info("推送至后台人工队列: eventId={}, riskLevel={}, userId={}", 
                event.getEventId(), event.getFinalRiskLevel(), event.getUserId());
        // 记录预警信息，后台工作人员可通过 API 接口查询待处理事件
        log.warn("【预警】需要人工介入 - 事件ID: {}, 用户ID: {}, 风险等级: {}", 
                event.getEventId(), event.getUserId(), event.getFinalRiskLevel());
    }
